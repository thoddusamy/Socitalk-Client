...
const ENDPOINT = 'https://socitalk.onrender.com'
    ...
    if (context.selectedChat) {
      try {
        ...
        const { data } = await axios.get(
          `${config.api}/messages/${context.selectedChat._id}`,
          cofigData
        )
        setMessages(data)
        setLoading(false)
          ...
          >
            ...
            {!context.selectedChat.isGroupChat ? (
              ...
              >
                ...
                <Text fontSize={'20px'}>
                  {getSender(context.user, context.selectedChat.users)}
                </Text>
              </Box>
            ) : (
            ...
            <FormControl onKeyDown={sendMessage} isRequired mt={3}>
              ...
              <Box display='flex' alignItems={'center'} gap={1}>
                <img
                  src={SmileyIcon}
                  className='emojiIcon'
                  onClick={() => setShowHideEmojiPicker(!showHideEmojiPicker)}
                />
                <Input
                  variant={'filled'}
                  bg='teal'
                ...
                <IconButton
                  bg={'teal'}
                  _hover={{ bg: 'teal' }}
                  onClick={handlesendMessageUsingArrow}
                >
